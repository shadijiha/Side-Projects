...
function Login(string $username, string $password)	{
	...
	if(mysqli_num_rows($query_run)>0)
	{
		...
		$_SESSION['user'] = new User($row['id'], $row['username'], $row['password'], $row['date'], $row['admin']);
...
function AddMessage(User $author, string $title, string $content, string $tags, string $date)	{

	$message = new Message(-1, $author->username, $title, $content, $tags, $date, "");
