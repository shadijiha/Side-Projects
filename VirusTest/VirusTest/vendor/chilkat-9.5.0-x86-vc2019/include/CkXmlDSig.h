// CkXmlDSig.h: interface for the CkXmlDSig class.
//
//////////////////////////////////////////////////////////////////////
...
// CLASS: CkXmlDSig
class CK_VISIBLE_PUBLIC CkXmlDSig  : public CkMultiByteBase
{
    private:

	// Don't allow assignment or copying these objects.
	CkXmlDSig(const CkXmlDSig &);
	CkXmlDSig &operator=(const CkXmlDSig &);

    public:
	CkXmlDSig(void);
	virtual ~CkXmlDSig(void);

	static CkXmlDSig *createNew(void);
	...
	// 
	bool CanonicalizeFragment(const char *xml, const char *fragmentId, const char *version, const char *prefixList, bool withComments, CkString &outStr);
	...
	// false, then XML comments are excluded from the output.
	// 
	const char *canonicalizeFragment(const char *xml, const char *fragmentId, const char *version, const char *prefixList, bool withComments);
	...
	// false, then XML comments are excluded from the output.
	// 
	bool CanonicalizeXml(const char *xml, const char *version, bool withComments, CkString &outStr);
	...
	// signatures found.
	// 
	bool LoadSignature(const char *xmlSig);
	...
	// signatures found.
	// 
	bool LoadSignatureBd(CkBinData &binData);
	...
	// signatures found.
	// 
	bool LoadSignatureSb(CkStringBuilder &sbXmlSig);
	...
	// validation.
	bool SetRefDataFile(int index, const char *path);
